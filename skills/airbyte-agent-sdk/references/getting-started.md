...
> **Note:** When working in the `airbyte-agent-sdk` repo, packages are already available—no installation needed.
...
> **Important:** When working in the `airbyte-agent-sdk` repo, place ALL files (`.env`, scripts, examples) in the specific connector directory, NOT the repo root.

| Working in... | Put files in... | Example |
|---------------|-----------------|---------|
| `airbyte-agent-sdk` repo | `connectors/{connector}/` | `connectors/gong/.env`, `connectors/gong/example.py` |
| Your own project | Project root | `.env`, `main.py` |

**In the airbyte-agent-sdk repo:**
